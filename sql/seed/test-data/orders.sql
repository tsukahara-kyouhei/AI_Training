WITH
    ...
    prepared AS (
        SELECT
            s.n,
            CASE
                WHEN s.n <= 55 THEN NOW() - MAKE_INTERVAL(
                    days => ((s.n - 1) % 7),
                    hours => (s.n % 6),
                    mins => (s.n % 50)
                )
        ...
        FROM
            ...
            LEFT JOIN active_members am ON am.rn = (
                (s.n - 1) % (
                    SELECT
                        COUNT(*)
                    FROM
...
SELECT
    ...
    END,
    CASE
        WHEN p.customer_type = 'member' THEN p.member_id
        ELSE NULL
    END,
    CASE
        WHEN p.customer_type = 'member' THEN p.m_last_name
        ELSE 'ゲスト' || LPAD(p.n::TEXT, 3, '0')
    END,
    CASE
        WHEN p.customer_type = 'member' THEN p.m_first_name
        ELSE '購入者'
    END,
    CASE
        WHEN p.customer_type = 'member' THEN p.m_last_name_kana
        ELSE 'ゲスト'
    END,
    CASE
        WHEN p.customer_type = 'member' THEN p.m_first_name_kana
        ELSE 'コウニュウシャ'
    END,
    CASE
    ...
    END,
    CASE
        WHEN p.customer_type = 'member' THEN p.m_email
        ELSE 'guest' || LPAD(p.n::TEXT, 3, '0') || '@example.com'
    END,
    LPAD((9000000000 + p.n)::TEXT, 10, '0'),
    CASE
        WHEN p.n % 3 = 0 THEN NULL
        ELSE LPAD((3100000000 + p.n)::TEXT, 10, '0')
    END,
    LPAD((1000000 + p.n)::TEXT, 7, '0'),
    (ARRAY['東京都', '神奈川県', '大阪府', '愛知県', '福岡県']) [((p.n - 1) % 5) + 1],
    (
        ARRAY['千代田区', '横浜市西区', '大阪市北区', '名古屋市中区', '福岡市博多区']
    ) [((p.n - 1) % 5) + 1],
    ...
    END,
    CASE
        WHEN p.n % 10 BETWEEN 7 AND 9  THEN jsonb_build_object(
            'payment_no',
            'CVS' || LPAD((100000 + p.n)::TEXT, 6, '0'),
            'payment_limit_date',
            TO_CHAR(
                (p.order_datetime + INTERVAL '3 day') AT TIME ZONE 'Asia/Tokyo',
                'YYYY-MM-DD'
            )
        )
        ELSE NULL
...
SELECT
    ...
    (
        (p.unit_price * p.quantity) + FLOOR((p.unit_price * p.quantity) * 0.10) + CASE
            WHEN p.shipping_method = 'assembly'
            AND p.assembly_available THEN p.base_assembly_fee * p.quantity
            ELSE 0
        END
    )::NUMERIC(12, 0),
    p.assembly_available,
    CASE
        WHEN p.shipping_method = 'assembly'
        AND p.assembly_available THEN p.base_assembly_fee
        ELSE 0
    END::NUMERIC(12, 0),
    NOW(),
    NOW()
...
SET
    ...
    tax_amount = FLOOR(
        (
            c.subtotal_amount + c.assembly_fee_total + c.shipping_fee
        ) * 0.10
    )::NUMERIC(12, 0),
    total_amount = (
        c.subtotal_amount + c.assembly_fee_total + c.shipping_fee + FLOOR(
            (
                c.subtotal_amount + c.assembly_fee_total + c.shipping_fee
            ) * 0.10
        )
...
INSERT INTO
    order_status_histories (order_id, status, changed_by_system, changed_at)
SELECT
    o.order_id,
    'received',
    'office-order',
...
FROM
    orders o
WHERE
    o.order_status IN ('processing', 'completed');
