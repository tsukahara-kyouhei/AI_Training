...
ALTER TABLE IF EXISTS orders ADD COLUMN IF NOT EXISTS discount_amount INT;
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    order_status VARCHAR(20) NOT NULL DEFAULT 'received'
        CHECK (order_status IN ('received', 'awaiting_payment', 'processing', 'completed', 'cancelled')),
    ...
    shipping_fax VARCHAR(12) CHECK (shipping_fax IS NULL OR shipping_fax ~ '^[0-9]{10,12}$'),
    ...
    payment_method VARCHAR(20) NOT NULL
        CHECK (payment_method IN ('bank_transfer', 'cash_on_delivery', 'convenience_store')),
    ...
    CHECK (payment_instruction IS NULL OR jsonb_typeof(payment_instruction) = 'object'),
    CHECK (
        (payment_method = 'convenience_store' AND payment_instruction IS NOT NULL)
        OR
        (payment_method <> 'convenience_store' AND payment_instruction IS NULL)
    )
);
...
CREATE TABLE IF NOT EXISTS order_status_histories (
    ...
    order_id BIGINT NOT NULL,
    status VARCHAR(20) NOT NULL CHECK (status IN ('received', 'awaiting_payment', 'processing', 'completed', 'cancelled')),
    changed_by_system VARCHAR(40) NOT NULL,
    changed_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_order_status_histories_order FOREIGN KEY (order_id)
        REFERENCES orders (order_id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_order_status_histories_order_changed_at
