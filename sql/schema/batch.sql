CREATE TABLE IF NOT EXISTS tax_rates (
    ...
    tax_rate_percent NUMERIC(5, 2) NOT NULL CHECK (
        tax_rate_percent > 0
...
CREATE TABLE IF NOT EXISTS popular_product_rankings (
    ...
    CONSTRAINT fk_popular_product_rankings_product FOREIGN KEY (product_id) REFERENCES products (product_id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_popular_rankings_product_date ON popular_product_rankings (product_id, ranking_date DESC);
...
CREATE TABLE IF NOT EXISTS recommended_related_products (
    ...
    CONSTRAINT uq_recommended_related_products_pair UNIQUE (
        recommendation_date,
        source_product_id,
        recommended_product_id
    ),
    CONSTRAINT fk_recommended_related_products_source FOREIGN KEY (source_product_id) REFERENCES products (product_id) ON DELETE CASCADE,
    CONSTRAINT fk_recommended_related_products_target FOREIGN KEY (recommended_product_id) REFERENCES products (product_id) ON DELETE CASCADE,
    CHECK (source_product_id <> recommended_product_id)
);

CREATE INDEX IF NOT EXISTS idx_recommended_related_products_source_date ON recommended_related_products (source_product_id, recommendation_date DESC);

CREATE INDEX IF NOT EXISTS idx_recommended_related_products_recommended ON recommended_related_products (recommended_product_id);
