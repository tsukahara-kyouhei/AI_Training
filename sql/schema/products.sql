CREATE TABLE IF NOT EXISTS colors (
    ...
    CONSTRAINT chk_colors_swatch_type CHECK (swatch_type IN ('solid', 'transparent_pattern'))
);
...
CREATE TABLE IF NOT EXISTS products (
    ...
    CHECK (
        ...
        OR (
            ...
            AND variation_name IS NULL
        )
    )
);

CREATE INDEX IF NOT EXISTS idx_products_category_sale_period ON products (category_id, sale_start_at, sale_end_at);

CREATE INDEX IF NOT EXISTS idx_products_sale_start_at ON products (sale_start_at DESC);

CREATE INDEX IF NOT EXISTS idx_products_variation_group ON products (variation_group_id);
...
CREATE TABLE IF NOT EXISTS product_variants (
    ...
    CONSTRAINT fk_product_variants_color FOREIGN KEY (color_id) REFERENCES colors (color_id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS idx_product_variants_in_stock ON product_variants (product_id)
WHERE
    stock_quantity > 0;
...
CREATE TABLE IF NOT EXISTS product_desk_attributes (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_product_desk_attributes_product FOREIGN KEY (product_id) REFERENCES products (product_id) ON DELETE CASCADE,
    CONSTRAINT fk_product_desk_attributes_top_shape FOREIGN KEY (top_shape_id) REFERENCES desk_top_shapes (top_shape_id) ON DELETE RESTRICT,
    CONSTRAINT fk_product_desk_attributes_taste FOREIGN KEY (taste_id) REFERENCES desk_tastes (taste_id) ON DELETE RESTRICT
);
...
CREATE TABLE IF NOT EXISTS product_chair_attributes (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_product_chair_attributes_product FOREIGN KEY (product_id) REFERENCES products (product_id) ON DELETE CASCADE,
    CONSTRAINT fk_product_chair_attributes_function FOREIGN KEY (function_id) REFERENCES chair_functions (function_id) ON DELETE RESTRICT,
    CONSTRAINT fk_product_chair_attributes_material FOREIGN KEY (material_id) REFERENCES chair_materials (material_id) ON DELETE RESTRICT,
    CONSTRAINT fk_product_chair_attributes_taste FOREIGN KEY (taste_id) REFERENCES chair_tastes (taste_id) ON DELETE RESTRICT
);
...
CREATE TABLE IF NOT EXISTS product_storage_attributes (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_product_storage_attributes_product FOREIGN KEY (product_id) REFERENCES products (product_id) ON DELETE CASCADE,
    CONSTRAINT fk_product_storage_attributes_usage FOREIGN KEY (usage_id) REFERENCES storage_usages (usage_id) ON DELETE RESTRICT,
    CONSTRAINT fk_product_storage_attributes_taste FOREIGN KEY (taste_id) REFERENCES storage_tastes (taste_id) ON DELETE RESTRICT
);
