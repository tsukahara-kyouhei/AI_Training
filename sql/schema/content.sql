CREATE TABLE IF NOT EXISTS announcements (
    ...
    CHECK (published_end_at IS NULL OR published_end_at > published_start_at)
);
CREATE INDEX IF NOT EXISTS idx_announcements_active_period
    ON announcements (published_start_at, published_end_at)
...
CREATE TABLE IF NOT EXISTS inquiries (
    ...
    phone VARCHAR(12) CHECK (phone IS NULL OR phone ~ '^[0-9]{10,12}$'),
    inquiry_type VARCHAR(20) NOT NULL
        CHECK (inquiry_type IN ('product', 'delivery_date', 'order', 'shipping', 'return_cancel', 'other')),
