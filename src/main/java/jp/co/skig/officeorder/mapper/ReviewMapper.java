...
public interface ReviewMapper {
    ...
    List<ProductReviewView> selectProductReviews(@Param("productId") String productId,
            @Param("limit") int limit,
            @Param("offset") int offset);

    // ②商品のレビュー件数を取得する (productId)
    // 戻り値は Long
    Long countProductReviews(@Param("productId") String productId);

    // ③商品のレビュー集計（平均評価と件数）を取得する (productId)
    // 戻り値は ReviewSummaryView
    ReviewSummaryView selectProductReviewSummary(@Param("productId") String productId);
