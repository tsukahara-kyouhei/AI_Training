...
public class CatalogController {
    ...
    public String searchResults(@RequestParam(name = "q", required = false) String keyword,
            ...
            @RequestParam(name = "size", defaultValue = "15") int size,
            Model model) {
        String normalizedKeyword = (keyword == null || keyword.isBlank()) ? null
        ...
        if (normalizedKeyword != null && normalizedKeyword.length() > 100) {
            ...
            model.addAttribute("sortValue", ProductSort.RECOMMENDED.value());
            model.addAttribute("inStockOnly", inStockOnly);
            model.addAttribute("selectedPriceBandIds", List.of());
            model.addAttribute("selectedColorKeys", List.of());
        ...
        model.addAttribute("keywordTooLong", false);
        ...
        if (result.productPage().totalCount() == 0) {
            model.addAttribute("suggestedProducts", productService.getTopRankedProducts(8));
        }
        return "pages/product-list-search-results";
    }
