...
public class CatalogController {
    ...
    public String productDetail(@PathVariable long productId,
        ...
        ProductReviewView memberReview = memberSessionService.currentMember(session)
                .flatMap(member -> reviewService.findMemberReview(member.memberId(), productId))
                .orElse(null);
    ...
    public String createReview(
            ...
            @Valid ProductReviewForm form,
            BindingResult bindingResult,
            HttpSession session,
            RedirectAttributes redirectAttributes) {

        MemberSessionUser member = memberSessionService.currentMember(session)
        ...
        if (bindingResult.hasErrors()) {
            redirectAttributes.addFlashAttribute(
                    "reviewError",
                    "レビューの入力内容を確認してください。");
            return "redirect:" + buildProductDetailPath(productId, false);
        }
     ...
     * 商品レビューを更新する。
     */
    @PostMapping("/products/{productId}/reviews/update")
